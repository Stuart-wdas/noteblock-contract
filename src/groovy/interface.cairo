...
pub struct Listing {
    ...
    pub price_per_song: u256,
    pub song_id: u256,
    pub copies: u256,
}

#[starknet::interface]
pub trait IGroovy<TContractState> {
    // Song functions
    ...
    fn mint_song(ref self: TContractState, song: Song, copies: u256); 
    ...
    fn buy_song(ref self: TContractState, id: u256, tip: u256);

    // Allows users to sell the song back into the market for the market price unless bought for another amount
    fn sell_song(ref self: TContractState, id: u256, to: ContractAddress);
